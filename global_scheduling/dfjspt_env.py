...
# from global_scheduling.dfjspt_data.SchedulingInstance import SchedulingInstance
# from dfjspt_data.dfjspt_data_generator import generate_a_complete_instance
# from global_scheduling.dfjspt_rule.dfjspt_rule9_FDDMTWR_EET_EET import rule9_a_makespan
...
class DfjsptMaEnv(MultiAgentEnv):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.empty_moving_time_matrix = factory_graph.unload_transport_time_matrix
        self.transport_time_matrix = factory_graph.loaded_transport_time_matrix
        ...
        for job_id in range(self.n_jobs):
            for operation_id in range(self.n_operations_for_jobs[job_id]):
                ...
                #     [0 if value <= 0 else 1 for value in self.instance.jobs[job_id].operations_matrix[job_id][operation_id]]
        ...
        self.rule_makespan_for_current_instance = MAX_JOBS * MAX_MACHINES * (dfjspt_params.max_prcs_time + dfjspt_params.max_tspt_time)
        # del instance
        ...
        if self.stage == 0:
            ...
            if self.schedule_done:
                ...
                global_schedule_file = os.path.dirname(os.path.abspath(__file__)) \
                                       + "/InterfaceWithLocal/global_schedules/global_schedule_" \
                                       + f"J{dfjspt_params.n_jobs}M{dfjspt_params.n_machines}" \
                                       + f"T{dfjspt_params.n_transbots}I{self.current_instance_id}.pkl"
