...
# import dfjspt_params
# from convert_schedule_to_json import convert_schedule_to_json
# from convert_schedule_to_class import convert_schedule_to_class
...
def rule1_single_makespan(
    ...
    while not done:
        if stage == 0:
            ...
            # job_actions_mask = (1 - legal_job_actions) * 1e8
            # jobs_last_finish_time = real_job_attrs[:, 2]
            # # jobs_last_finish_time = real_job_attrs
            # jobs_last_finish_time += job_actions_mask
            # EST_job_action = {
            #     "agent0": np.argmin(jobs_last_finish_time)
            # }
    ...
    # make_span = env.final_makespan
    # global_schedule = convert_schedule_to_json(
    #     file_name=f"global_schedule_J{dfjspt_params.n_jobs}M{dfjspt_params.n_machines}T{dfjspt_params.n_transbots}I{instance_id}.json",
    #     result_start_time_for_jobs=env.result_start_time_for_jobs,
    #     result_finish_time_for_jobs=env.result_finish_time_for_jobs,
    #     machine_routes=env.machine_routes,
    #     transbot_routes=env.transbot_routes
    # )
    # global_schedule = convert_schedule_to_class(
    #     file_name=f"global_schedule_J{dfjspt_params.n_jobs}M{dfjspt_params.n_machines}T{dfjspt_params.n_transbots}I{instance_id}.pkl",
    #     result_start_time_for_jobs=env.result_start_time_for_jobs,
    #     result_finish_time_for_jobs=env.result_finish_time_for_jobs,
    #     machine_routes=env.machine_routes,
    #     transbot_routes=env.transbot_routes
